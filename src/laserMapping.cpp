...
PointCloudXYZI::Ptr pcl_wait_save_out(new PointCloudXYZI());
...
void map_incremental(float resolution) {

    int feats_down_size = feats_down_body->points.size();
    // 往地图里更新的点。
    PointVector PointToAdd;
    PointToAdd.reserve(feats_down_size);
    // 两个哈希集合，存储 occupy 体素与 unoccupy 体素
    std::unordered_set<Eigen::Matrix<int, 3, 1>, hash_vec<3>> pass, hit;

    std::vector<size_t> index(feats_down_size);
    ...
    std::for_each(std::execution::unseq, index.begin(), index.end(), [&](const size_t &i) {
        ...
        V3D hit_local(point_body.x, point_body.y, point_body.z);
        V3D hit_global(state_point.rot * (state_point.offset_R_L_I*hit_local + state_point.offset_T_L_I) + state_point.pos);
        // 转换为体素坐标
        auto key = (hit_global / resolution).array().round().template cast<int>();
        // 返回 value 是一个迭代器
        auto iter = hit.find(key);
        // if new
        if (iter == hit.end()) {
            // 保存新的体素信息
            hit.emplace(key);
        ...
        while (disx > 0.5) {
            ...
            disy -= dy;
            disz -= dz;
            V3D pass_local(disx, disy, disz);
            V3D pass_global(state_point.rot * (state_point.offset_R_L_I*pass_local + state_point.offset_T_L_I) + state_point.pos);
            // 转换为体素坐标
            auto key = (pass_global / resolution).array().round().template cast<int>();
            // 返回 value 是一个迭代器
            auto iter = pass.find(key);
            // if new
            if (iter == pass.end()) {
                // 保存新的体素信息
                pass.emplace(key);
...
void h_share_model(state_ikfom &st, esekfom::dyn_share_datastruct<double> &ekfom_data) {
    ...
    auto R_wl = (st.rot * st.offset_R_L_I).cast<float>();
    auto t_wl = (st.rot * st.offset_T_L_I + st.pos).cast<float>();
    std::vector<size_t> index(feats_down_size);
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const size_t &i) {
        ...
        // 如果最近邻的点数小于 NUM_MATCH_POINTS，则认为该点不是有效点
        // 需不需要再加一个最近邻的距离判断？？？？？
    ...
    index.resize(effct_feat_num);
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const size_t &i) {
        // 拿到有效点云的 LiDAR 坐标
        const PointType &laser_p = laserCloudOri->points[i];
        V3D point_this_be(laser_p.x, laser_p.y, laser_p.z);
        M3D point_be_crossmat;
        point_be_crossmat << SKEW_SYM_MATRX(point_this_be);  // LiDAR 中，点云的 ^ 矩阵
        // 转换到 IMU 坐标系下
        V3D point_this = st.offset_R_L_I * point_this_be + st.offset_T_L_I;
        M3D point_crossmat;
        point_crossmat << SKEW_SYM_MATRX(point_this);  // IMU 中，点云的 ^ 矩阵
...
int main(int argc, char** argv) {
    ...
    neal::logger(neal::LOG_INFO, "voxel lio start.");
    ...
    nh.param<double>("mapping/filter_size_map",filter_size_map_min,0.05);  // 地图分辨率
    nh.param<double>("mapping/filter_size_surf",filter_size_surf_min,0.5);  // 每帧数据降采样，取 0.5，太小会崩溃
    ...
    nh.param<double>("preprocess/blind",param_blind,0.5);
    nh.param<int>("preprocess/scan_line",param_scans,1);
    nh.param<int>("point_filter_num", param_filters,2);  // 采样间隔，即每隔 point_filter_num 个点取 1 个点
    ...
    std::fill(epsi, epsi+23, 0.001);  // 迭代收敛条件。
    ...
    while (status) {
        ...
        map_incremental(ivox_options_.resolution_);
